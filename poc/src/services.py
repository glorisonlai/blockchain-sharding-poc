...
""" Number of parallel miners """
""" 
NOTE: 1 process is used for each calling function 
For Sharding tests, 1 process + num_shards are already allocated
To calculate how many miners to use, calculate CPU Processes - Num_Shards - 1
Take care to not allocate fewer miners than num_shards, or a shard won't mine any transactions
""" 
allocated_miners = 4
...
def test_serial(transactions: int) -> int:
	...
	mining(allocated_miners)
	return mining.last_time



def test_shard(transactions: int) -> int:
	""" Generate transactions in sharded network, then mine all at once """
